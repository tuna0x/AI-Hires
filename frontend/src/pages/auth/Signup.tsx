...
const schema = z.object({
  name: z.string().trim().min(2, "Họ tên quá ngắn").max(80),
...
export default function Signup() {
  ...
  async function onGoogle() {
    ...
    if (r.error) { setBusy(false); toast.error("Đăng ký Google thất bại"); }
  }

  return (
    <AuthShell
      title="Tạo tài khoản"
      subtitle="Miễn phí trọn đời. Không cần thẻ thanh toán."
      seoTitle="Đăng ký — CareerAI"
      seoDescription="Tạo tài khoản CareerAI để phân tích CV và chuẩn bị phỏng vấn."
      path="/signup"
    ...
    >
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="relative text-center text-xs text-muted-foreground">
          <span className="bg-background px-2 relative z-10">hoặc đăng ký bằng email</span>
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1.5">Tối thiểu 8 ký tự. Tránh mật khẩu phổ biến hoặc đã bị lộ.</p>
        </div>
        ...
        <p className="text-xs text-muted-foreground text-center">Bằng việc đăng ký, bạn đồng ý với Điều khoản và Chính sách bảo mật của chúng tôi.</p>
      </form>
    </AuthShell>
  );
}